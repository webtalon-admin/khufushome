...
Deno.serve(async (_req) => {
	try {
		...
		// Fetch from Oct 2020 (earliest research data) to now
		...
		console.log(`Fetching BTC/AUD prices from CoinGecko...`);

		const response = await fetch(url);
		if (!response.ok) {
			const errText = await response.text();
			throw new Error(
				`CoinGecko API returned ${response.status}: ${errText.slice(0, 200)}`,
			);
		}

		const data: MarketChartResponse = await response.json();
		console.log(`Received ${data.prices.length} daily price points`);
